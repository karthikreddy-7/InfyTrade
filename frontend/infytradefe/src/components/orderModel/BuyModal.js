  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-40 flex items-center justify-center">
      <div className="bg-white p-6 rounded-lg shadow-md w-96">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Buy {company}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-gray-600 mb-1">Price (Bid Price):</label>
              <input
                type="number"
                value={price}
                readOnly // Make the input read-only
                className="border-2 border-blue-300 p-3 rounded-lg w-full bg-gray-100 text-gray-800"
              />
            </div>
          </div>
